...
2. KCSC 검색 키워드 도출: KCSC 공식 사이트 검색창에서 검색했을 때 해당 기준이 바로 검색되는 실질적인 전문 건설 기술 단어(예: "동바리 수평연결재", "비탈면 가배수로", "작업발판 틈새", "철근 피복두께" 등) 도출
...
- 원칙: 틀리기 쉬운 임의의 가짜 번호를 단정 짓지 말고, KCSC 검색창에서 직접 조항을 찾을 수 있는 정확한 '검색 키워드'를 제공할 것
...
\`\`\``;
    ...
    // AI가 도출한 실질적 건설 키워드로 KCSC 공식 통합검색 딥링크 동적 생성
    ...
    const kcscLinksMarkdown = rawKeywords.map((kw) => {
      ...
      const searchUrl = `https://www.kcsc.re.kr/Search/ListCodes?searchKeyword=${encodeURIComponent(trimmed)}`;
      return `• 🔍 **[KCSC 공식 기준검색: '${trimmed}' 바로가기 ↗](${searchUrl})**`;
    }).join("<br/>");
